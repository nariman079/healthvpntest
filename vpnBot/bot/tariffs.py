...
_SERVICES: Dict[str, TariffService] = {
    "standard": TariffService(
        ...
        plans=[
            TariffPlan("test", "🧪 Тест — 1₽ (1 дня)", days=1, amount_minor=100, admin_only=True),
            TariffPlan("1m", "1 месяц — 119₽", days=30, amount_minor=11900),
            TariffPlan("3m", "3 месяца — 295₽", days=90, amount_minor=29500),
            TariffPlan("6m", "6 месяцев — 479₽", days=180, amount_minor=47900),
        ],
    ),
    "obhod": TariffService(
        key="obhod",
        ...
        description=(
            "Альтернативный канал. Позволяет ускорить мобильный интернет в регионах,"
            "⚠️ Мобильный интернет ускоряется не на всех операторах."
        ),
        ...
        plans=[
            ...
            TariffPlan("1d", "1 день — 19₽", days=1, amount_minor=1900),
            TariffPlan("1w", "1 неделя — 59₽", days=7, amount_minor=5900),
            TariffPlan("1m", "1 месяц — 119₽", days=30, amount_minor=11900),
            TariffPlan("3m", "3 месяца — 299₽", days=90, amount_minor=29900),
            TariffPlan("6m", "6 месяцев — 499₽", days=180, amount_minor=49900),
        ],
    ),
}
